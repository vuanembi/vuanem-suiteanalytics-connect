SELECT *,
        CASE WHEN OPENNING_DAY IS NULL THEN FIRST_TRAFFIC_DATE
...
FROM
    (
        SELECT
            ...
            CASE 
            	WHEN LOCATIONS.CLOSE_DATE IS NULL OR LOCATIONS.CLOSE_DATE > CURRENT_DATE() THEN '0'
                ELSE '1'
            END AS 'ISCLOSE'
        FROM "Vua Nem Joint Stock Company"."Vua Nem - Storehouse Officer".LOCATIONS LOCATIONS
        ...
        LEFT JOIN "Vua Nem Joint Stock Company"."Vua Nem - Storehouse Officer".STORE_MODEL STORE_MODEL ON STORE_MODEL.LIST_ID = LOCATIONS.STORE_MODEL_ID
    ) AS LOCATION
    LEFT JOIN (
        SELECT
